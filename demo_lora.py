#!/usr/bin/env python3
"""
3D Point Cloud Inference and Visualization Script with LoRA Support

This script performs inference using the ARCroco3DStereo model with LoRA fine-tuning
and visualizes the resulting 3D point clouds with the PointCloudViewer.

Usage:
    python demo_lora.py [--model_path MODEL_PATH] [--lora_path LORA_PATH] [--seq_path SEQ_PATH] 
                        [--size IMG_SIZE] [--device DEVICE] [--vis_threshold VIS_THRESHOLD] 
                        [--output_dir OUT_DIR]

Example:
    python demo_lora.py --model_path src/cut3r_512_dpt_4_64.pth \
        --lora_path src/checkpoints_lora/dpt_512_lora_r16_a16/lora_weights_final.pth \
        --seq_path ../VIFT/data/kitti_data/sequences/00/image_2/ --device cuda --size 512
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run 3D point cloud inference and visualization using ARCroco3DStereo with LoRA."
    )
    parser.add_argument(
        "--model_path",
        type=str,
